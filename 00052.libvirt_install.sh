...
# configures it with a provided tar.bz2 archive containing the following files:
# - virt domain ( file beginning with: <domain type='kvm'> ) 
# - virt storage pool file ( file beginning with: <pool type='dir'> )
# - libvirtd.conf ( Master libvirt daemon configuration file )
# - SLIC ( ACPI SLIC table )

# This script will also install samba and adds one shared directory.
#
# References:
...
if [ $# != 1 ]; then
  echo "please provide a path with the configuration for libvirt"
else
  libvconf="$(basename -- "$1")"
fi
...
SMB_USER="$(whoami)"
SMB_PWD="$(tr -dc A-Z0-9 < /dev/urandom | head -c 10 | xargs)"
...
sudo apt-get update && sudo apt-get -y install qemu-kvm virt-manager \
                            ...
                            virtinst qemu-guest-agent spice-vdagent

## verify if virt settings archive is tar.bz2 file.
if ! bzip2 -tvq "$1" 2>&1  >/dev/null | grep -q "tar.bz2: ok"; then
  echo "Provided file ${libvconf} is not a .tar.bz2 file. Aborting."
  exit 1
fi
...
echo "The following configuration files were found:"
...
printf "\n...extracting kvm domain name: "
for file in * ; do
  if [ -e "${file}" -a "${file##*.}" = "xml" ]; then
    if head -n 1 $file | grep -q "<domain type='kvm'>"; then
      rname=$(grep "<name>" "${file}" | sed -n 's:.*<name>\(.*\)</name>.*:\1:p')
      printf "$rname\n"
      if virsh list --all | awk '{print $2}' | tail -n +2 | grep -q ${rname}; then
        printf "domain already installed on this system. Aborting...\n"
        rm -Rf "$tmpdir"
        exit 1
      fi
    fi
...
# ... domain.xml  <-- copy not required. Import with "virsh define --file ..."
...
if [ -z "$( sudo getent group | grep libvirt | grep "$(whoami)")" ]; then
...
if ! virsh list --all | grep -q "${domain_name}"; then
  ...
  if [ "$?" != "0" ]; then
    echo "If you receive the following error message:"
    echo "  Emulator '/usr/bin/kvm-spice does not support 'kvm', then please"
    echo "verify if in the system bios the following settings are enabled:"
    echo "Bios -> Security -> Virtualization"
    echo "  Kernel DMA Protection: Off"
    echo "  Intel Virtualization Technology: On"
    echo "  Intel VT-d Feature: On"
    echo "Then save and exit and retry this script."
    exit 1
  fi
else
...
# after starting windows, you can verify the activation status in windows with:
#   slmgr /xpr
# If it respond with the text below, all is OK
...
# If the password does not work, type the following command:
...
if ! grep -qe "^\[${SMB_USER}\]" "${smbconf}"; then
  ...
  q="[${SMB_USER}]\n"
  q="$q    path = ${SMB_SHARE}\n"
  q="$q    acl allow execute always = True\n"
  q="$q    browseable = no\n"
  q="$q    read only = no\n"
...
## Show info about HiDPI screen settings
# Make sure you have installed the following packages on your host with:
#   sudo apt-get install qemu-guest-agent spice-vdagent
# Then switch to Video Virtio at the video adapters setting in the Virtual Machine
# settings, and click Apply. (Re)start the VM. 
# Note that it will still leave a black bar.
# Make sure to shut down the VM first before changing.
# Open the VM’s hardware details in virt-manager.
# Go to the Video tab (or Display).Change the Model to Virtio 
printf "\n\nIf you are using a HiDPI screen, you may have to look at the following:\n"
printf "Install Windows SPICE Guest Tools as shown here:\n"
printf "#  https://superuser.com/questions/132322/how-to-increase-the-visualized-screen-resolution-on-qemu-kvm\n"
printf "Then, in the Virtual Machine Manager for Windows 10, set the following:\n"
printf "  View -> Scale Display -> Always\n"
printf "  View -> Scale Display -> Auto resize VM with window\n\n"
printf "Then in Windows 10, right click on Desktop -> Display settings -> Scale and Layout:\n"
printf "  Change the size of text, apps and other items: 100%\n"
printf "  Display resolution: 1920 x 1080\n\n"
printf "The Virtual Machine Manager Option in View -> Resize to VM has slight problems.\n"
printf "It will give a black border around the windows 10 screen.\n"
printf "To solve, click and drag the corner of the Virtual Machine window to hide the black border.\n\n."
...
q="${q}You are done. Now the network map should appear."
